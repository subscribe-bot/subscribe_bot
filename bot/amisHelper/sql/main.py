...
_session_local = sessionmaker(autoflush=True, bind=engine)
...
def _sql_exec(sentence:str, *args)->sqlalchemy.engine.CursorResult:
  with _session_local() as _session:
    ...
    ret=_session.execute(text("".join(ls)), dict(zip(map(str, range(1,len(binds)+1)),binds)))
    _session.commit()
    return ret
