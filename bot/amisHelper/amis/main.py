...
import regex as re
from fastapi import APIRouter, Body, FastAPI, Path, Request, Response
...
def startAmis(app:FastAPI, defaultPath=None, password=None):
  ...
  def amisLoad(title:str, json:str):
    ...
    return amisTemplate.replace('{%title%}',title).replace('{%json%}',json).replace('{%CDN%}',"https://gcore.jsdelivr.net/npm/amis@2.4.0/sdk")
